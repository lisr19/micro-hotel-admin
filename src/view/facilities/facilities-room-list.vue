<template>
  <div>
    ...
    <!--编辑服务Modal-->
    <Modal v-model="isEdit" @on-cancel="cancelEditModal" title="服务编辑：" width="900">
      <Form :model="editForm" ref="editForm" :rules="rules" :label-width="110">
        <Card>
          <FormItem label="服务：" prop="facilityServiceId">
            <Select clearable filterable v-model="editForm.facilityServiceId" placeholder="请输入服务类型名称查询">
              <Option v-for="(option, index) in serviceData" :value="option.id" :key="index">{{option.name}}</Option>
            </Select>
          </FormItem>
          <FormItem label="所属类型：" prop="flag">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="doRoomFacilitiesEdit">确认</Button>
      </div>
    </Modal>
  </div>
</template>
        ...
        this.$refs.editForm.validate(async (valid) => { // 表单校验
          if (valid) { // 校验成功
            ...
            if (Object.keys(data).length !== 0) { // 有修改
              data.id = form.id
              let res = await doRoomFacilitiesEdit(data)

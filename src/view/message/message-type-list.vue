<template>
  <div>
    ...
    <!--添加消息类型Modal-->
    <Modal v-model="isAdd" @on-cancel="cancelAddModal" title="消息类型添加：" width="500">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="doTypeAdd">确认</Button>
      </div>
    </Modal>

    <!--编辑消息类型Modal-->
    <Modal v-model="isEdit" @on-cancel="cancelEditModal" title="消息类型添加：" width="500">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="doTypeEdit">确认</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 删除消息类型
      async doTypeDel(params) {
        // let res = await doUserDel(params)
        // if (res.code === 200) {
        //   this.$Message.success('删除成功')
        //   this.findTypeData(this.searchOption)
        // } else {
        //   this.$Message.error(res.message)
        // }
      },

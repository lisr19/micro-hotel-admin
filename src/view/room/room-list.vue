      ...
      <Row>
        ...
        <Col span="3" offset="1">
          <Input clearable icon="search" v-model="hotelName" @on-change="handleSearchByName"
                 placeholder="输入酒店/民宿名称搜索"/>
        </Col>
        <Col span="3" offset="1">
          <Input clearable icon="search" v-model="roomType" @on-change="handleSearchByType"
                 placeholder="输入房型搜索"/>
        </Col>
      </Row>
    </Card>
    ...
    methods: {
      ...
      openEditModal(params) {
        ...
        if(params.flag === '1'){
          ...
          this.remoteMethod2(params.hotelName)
        }
      ...
      //根据房型名称查询
      handleSearchByType(){
        if (this.roomType) { // 判断搜索条件是否为空
          this.searchOption.roomType = this.roomType
        } else { // 重置搜索内容
          this.searchOption.roomType = ''
        }
